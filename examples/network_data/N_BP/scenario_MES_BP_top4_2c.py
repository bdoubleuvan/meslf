...
q_c = .7*q_hl_scen[2] # used for consistency throughout the other scenario's
Eg_c = GHV*q_c
...
eta_CHP = np.array([2*P_c/Eg_c, 2*-phi_scen[0]/Eg_c]) # phi_scen[0] is used for consistency throughout the other scenario's
eta_GB = eta_CHP[1]
phi_c_GB = -phi_scen[0] #[W]
q_c_GB = phi_c_GB/(eta_GB*GHV)
q_c_CHP = 1.1*q_c - q_c_GB # additional factor required, otherwise there is no heat power coming from the CHP
phi_c_CHP = eta_CHP[1]*(GHV*q_c_CHP - P_c/eta_CHP[0])
m_c_CHP = phi_c_CHP / (Cp * (Ts_scen[1] - Tr_scen[1]))
q1_load = q_hl_scen[1] - q_c_GB
q2_load = q_hl_scen[2] - q_c_CHP
To_GB = Ts_scen[0] #required
To_CHP = Ts_scen[1] #required

m1_load = m_c_CHP + m_hl_scen[1]
To_1 = (m_c_CHP*Tr_scen[1] + m_hl_scen[1]*Tr_hl_scen[1])/m1_load
phi1_load = m1_load * Cp * (Ts_scen[1] - To_1)
print('coupling parameters:')
print('q_c = {} kg/s'.format(q_c))
print('q_c GB = {} kg/s'.format(q_c_GB))
print('q_c CHP = {} kg/s'.format(q_c_CHP))
print('q_c_GB+q_c_CHP= {} kg/s'.format(q_c_GB+q_c_CHP))
print('Eg_c = {} W'.format(Eg_c))
print('P_c = {} W'.format(P_c))
print('Eg_CHP = {} W'.format(GHV*q_c_CHP))
print('Eg to P CHP = {} W'.format(P_c/eta_CHP[0]))
...
def create_network():
    ...
    # gas network
    g0 = GasNode('gn0',node_type=0,x=xg0,y=yg0,p=pg_scen[0]) # reference node
    g1 = GasNode('gn1',node_type=1,x=xg1,y=yg1,q=q1_load) # load node
    g2 = GasNode('gn2',node_type=1,x=xg2,y=yg2,q=q2_load) # load node
    ...
    # heat network
    h0 = HeatNode('hn0',node_type=5,x=xh0,y=yh0,p=ph_scen[0]) # reference junction
    h1 = HeatNode('hn1',node_type=3,x=xh1,y=yh1,p=ph_scen[1],Tr_hl=To_1,dphi=phi1_load) # load reference node (sink)
    h1.half_links[0].set_type('heat_exchanger',{'carrier':water})
    h2 = HeatNode('hn2',node_type=1,x=xh2,y=yh2,Tr_hl=Tr_hl_scen[2],dphi=phi_scen[2]) # load node (sink)
    ...
    hlc1 = HeatLink('hl_c',cn1,h1,link_params={'carrier':water},bc_type=6,Tsstart=To_CHP) # To of coupling (source) is known
...
def initialize_network(gas_net,elec_net,heat_net,het_net):
    ...
    ph_init = np.array([99.5])*rho_w*grav_const #[Pa]
    Ts_init = np.array([100.,99.6,99.3]) #[C]
    ...
    phic_init = -phi_scen[0]*np.ones(len(unknown_dphi_links))
...
def save_scenario():
    ...
    nodes.to_pickle(os.path.join(dir_path,'MES_BP_nodes_top4_2c.pkl'))
    links.to_pickle(os.path.join(dir_path,'MES_BP_links_top4_2c.pkl'))
    halflinks.to_pickle(os.path.join(dir_path,'MES_BP_halflinks_top4_2c.pkl'))
...
def table():
    ...
    with open(os.path.join(path_to_data,'coupling_data_BP_top4_2c.txt'), "w") as table:
        table.write(r'\multirow{2}{*}{Top. 4}' )
