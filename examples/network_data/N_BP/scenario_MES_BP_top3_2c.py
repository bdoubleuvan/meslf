...
q2_load = q_hl_scen[2] - q_c_GB - q_c_CHP
...
if q2_load < 0:
    raise ValueError('Gas node 2 is acting as a source instead of a sink')
...
def create_network():
    ...
    # heat network
    h0 = HeatNode('hn0',node_type=5,x=x0,y=y0,p=ph_scen[0]) # reference junction
    h1 = HeatNode('hn1',node_type=3,x=x1,y=y1,p=ph_scen[1],Tr_hl=To_1,dphi=phi1_load) # load reference node (sink)
    h1.half_links[0].set_type('heat_exchanger',{'carrier':water})
    h2 = HeatNode('hn2',node_type=1,x=x2,y=y2,Tr_hl=Tr_hl_scen[2],dphi=phi_scen[2]) # load node (sink)
...
def save_scenario():
    ...
    nodes.to_pickle(os.path.join(dir_path,'MES_BP_nodes_top3_2c.pkl'))
    links.to_pickle(os.path.join(dir_path,'MES_BP_links_top3_2c.pkl'))
    halflinks.to_pickle(os.path.join(dir_path,'MES_BP_halflinks_top3_2c.pkl'))
...
def table():
    ...
    with open(os.path.join(path_to_data,'coupling_data_BP_top3_2c.txt'), "w") as table:
        table.write(r'\multirow{2}{*}{Top. 3}' )
