"""Make network, with one solution scenario, for the multi-carrier network of the Benchmark Problem (BP), topology 2.
...
q_c = .7*q_hl_scen[2] # part of the load of node 2 is redirected to the coupling (Even though coupling happens at node 1 and 2, this value is still used for consistency with the other topologies)
Eg_c = GHV*q_c
P1_load = 1*MW #[W]
...
def create_network():
    ...
    # gas network
    g0 = GasNode('gn0',node_type=0,x=x0,y=y0,p=pg_scen[0]) # reference node
    g1 = GasNode('gn1',node_type=1,x=x1,y=y1,q=q1_load) # load node
    g2 = GasNode('gn2',node_type=1,x=x2,y=y2,q=q2_load) # load node
...
def save_scenario():
    ...
    nodes.to_pickle(os.path.join(dir_path,'MES_BP_nodes_top2_2c.pkl'))
    links.to_pickle(os.path.join(dir_path,'MES_BP_links_top2_2c.pkl'))
    halflinks.to_pickle(os.path.join(dir_path,'MES_BP_halflinks_top2_2c.pkl'))
...
def table():
    ...
    with open(os.path.join(path_to_data,'coupling_data_BP_top2_2c.txt'), "w") as table:
        table.write(r'\multirow{2}{*}{Top. 2}' )
