"""Example of a MES with 3 nodes per carrier. Based on the example in Shabanpourt-Haghighi and Seifi.  Optimal flow."""
from examples import MES_Shabanpour as MES
import warnings
import numpy as np
from meslf.utils.constants import mm, bar, hour, kV, MW, MSCM, BTU, MBTU
from meslf.utils.hide_print import HiddenPrints
from meslf.utils.post_processing import error, max_error_ind
...
variable_names_u = [r'$p^g_2',r'$|V_2|',r'$p^h_2',r'$T^s_{2^c2^h}']
variable_names = variable_names_u +  [r'$q_{0,0}$',r'$q_{01}$',r'$q_{02}$',r'$q_{32}$',r'$q_{13}$',r'$p^g_{1}$',r'$p^g_{3}$',r'$\delta_1$',r'$\delta_2$',r'$|V_1|$',r'$m_{01}$',r'$m_{02}$',r'$m_{12}$',r'$m_{1}$',r'$m_{2}$',r'$p^h_1$',r'$T^s_0$',r'$T^s_1$',r'$T^s_2$',r'$T^r_0$',r'$T^r_1$',r'$T^r_2$',r'$q_{0^g0^c}$',r'$q_{0^g1^c}$',r'$q_{0^g2^c}$',r'$P_{0^c0^e}$',r'$P_{2^c2^e}$',r'$Q_{0^c0^e}$',r'$Q_{2^c2^e}$',r'$m_{1^c0^h}$',r'$m_{2^c2^h}$',r'$-\Delta\varphi_{1^c0^h}$',r'$-\Delta\varphi_{2^c2^h}$']
...
def objective_function(y,y_ind,a=np.array([0,0,0,0,0]),b=np.array([.01*MES.GHV,.3,.3,.04,.05]),c=np.array([1e-6*(MES.GHV)**2,3e-5,2e-5,4e-4,4.5e-4]),scale_var=None,scale_var_params=None,fb=None):
    ...
    """
    # print('Variables in obj: {}'.format(y[np.array(y_ind)]))
...
def h(y, nlsys=None, Dh=None,scale_var=None,scale_var_params=None):
...
def h_der(y,nlsys=None, Dy_inv=None, Dh=None,scale_var=None,scale_var_params=None):
    ...
    dh_dy[1:,5:] = J_lf #dF_dxlf
    # derivative to control pressure of gas network
    nlsysg = nlsys.nlsystemsg[0]
    network_g = nlsysg.gasnetwork
    xg = xmes[:len(nlsys.xg_entries)]
    Jg_full = nlsysg.J_dense(xg,return_full=True)
    pg2_ind = len(network_g.links) + 2
    Fg_ind = nlsysg.ind_Fn + [len(network_g.nodes)+ind for ind in nlsysg.ind_Fl]
    # print('Fg ind = {}\npg2 ind = {}'.format(Fg_ind,pg2_ind))
    # plt.figure('Jgg full')
    # plt.spy(Jg_full)
    # plt.plot([-.5,10.5],[3.5,3.5])
    # plt.plot([6.5,6.5],[-.5,10.5])
    dh_dy[1:1+len(Fg_ind),0] = Jg_full[Fg_ind,pg2_ind].ravel() #dFg_dpg2
    ...
    dh_dy[1+len(nlsys.Fg_entries):1+len(nlsys.Fg_entries)+len(Fe_ind),1] = Je_full[Fe_ind,V2_ind].ravel() # dFe_dV2
    # derivative to control pressure of heat network
    nlsysh = nlsys.nlsystemsh[0]
    network_h = nlsysh.heatnetwork
    xh = xmes[len(nlsys.xg_entries)+len(nlsys.xe_entries):len(nlsys.xg_entries)+len(nlsys.xe_entries)+len(nlsys.xh_entries)]
    Jh_full = nlsysh.J_dense(xh,return_full=True)
    Nh = len(network_h.nodes)
    Eh = len(network_h.links)
    Th = len(network_h.half_links)
    ph2_ind = Eh + Th + Nh - 1
    Fh_ind = nlsysh.Fm + [Nh+ind for ind in nlsysh.Fdeltap] + [Nh+Eh+ind for ind in nlsysh.FTs] + [Nh+Eh+Nh+ind for ind in nlsysh.FTr] + [Nh+Eh+2*Nh+ind for ind in nlsysh.Fphi] + [Nh+Eh+2*Nh+Th+ind for ind in nlsysh.FdT]
    dh_dy[1+len(nlsys.Fg_entries)+len(nlsys.Fe_entries):1+len(nlsys.Fg_entries)+len(nlsys.Fe_entries)+len(Fh_ind),2] = Jh_full[Fh_ind,ph2_ind].ravel() #dFh_dph2
    # derivatives of slack equation in gas node
    dh_dy[0,[4,5,6,5+len(nlsys.xg_entries)+len(nlsys.xe_entries)+len(nlsys.xh_entries),5+len(nlsys.xg_entries)+len(nlsys.xe_entries)+len(nlsys.xh_entries)+1]] = -1 #dG_dy
    # derivatives to control temperature
    m2c_ind = 5+len(nlsys.xg_entries)+len(nlsys.xe_entries)+len(nlsys.xh_entries)+8 # index of m2c within y
    m2c = y[m2c_ind]
    dh_dy[-1,3] = MES.water.get_Cp(scale_var=nlsys.scale_var,scale_var_params=nlsys.scale_var_params)*m2c #dFdphi2c_dTs2c
    if m2c > 0:
        dh_dy[1+len(nlsys.Fg_entries)+len(nlsys.Fe_entries)+len(nlsys.F_m_nodes)+len(nlsys.F_deltap_links)+2,3] = -m2c #dFTs2_dTs2c
    if nlsys.scale_var == 'matrix':
        dh_dy = Dh.dot(dh_dy.dot(Dy_inv))
    # plt.figure('Jacobian')
    # plt.spy(dh_dy)
    # plt.plot([-.5,36.5],[.5,.5])
    # plt.plot([-.5,36.5],[7.5,7.5])
    # plt.plot([-.5,36.5],[13.5,13.5])
    # plt.plot([-.5,36.5],[27.5,27.5])
    # plt.plot([3.5,3.5],[-.5,32.5])
    return dh_dy

def update_bc(het_net, gas_net, elec_net, heat_net, pg2, V2, ph2, Ts2c, scale_var=None,scale_var_params=None):
    """Update the boundary conditions of the MES, based on the control variables of OF"""
    if scale_var == 'per_unit':
        pg2 = pg2*scale_var_params.get('pbase')
        V2 = V2*scale_var_params.get('Vbase')
        ph2 = ph2*scale_var_params.get('pbase')
        Ts2c = Ts2c*scale_var_params.get('Tbase')
    gas_net.nodes[2].p = pg2
    het_net.nodes[2].p = pg2
    elec_net.nodes[2].V = V2
    het_net.nodes[6].V = V2
    heat_net.nodes[2].p = ph2
    het_net.nodes[9].p = ph2
    heat_net.links[-1].Tsstart = Ts2c
    het_net.links[-1].Tsstart = Ts2c
    heat_net.links[-1].Tsend = Ts2c
    het_net.links[-1].Tsend = Ts2c
    return het_net, gas_net, elec_net, heat_net

def run_optimal_load_flow(u_lb=np.array([1*bar,.8*MES.Vbase_shabanpour,1*MES.rho_w*MES.grav_const,100]),u_ub=np.array([MES.pg0,1.2*MES.Vbase_shabanpour,MES.ph0,135]),u_init=np.array([.9*MES.pg0,1.1*MES.Vbase_shabanpour,.9*MES.ph0,130]),slack_lb=np.array([-80*MSCM*MES.rhon_g/hour]),slack_ub=np.array([0]),slack_init=np.array([-50*MSCM*MES.rhon_g/hour]),xLF_lb=np.array([-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,1*bar,1*bar,-np.pi,-np.pi,0.8*MES.Vbase_shabanpour,-100,-100,-100,0,0,1*MES.rho_w*MES.grav_const,60,60,60,0,0,0,0,0,0,0,0,-50*MES.Sbase_shabanpour,-50*MES.Sbase_shabanpour,0,0,0,0]),xLF_ub=np.array([80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,60*bar,60*bar,np.pi,np.pi,1.2*MES.Vbase_shabanpour,100,100,100,200,200,6000*MES.rho_w*MES.grav_const,135,135,135,60,60,60,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,60*MES.Sbase_shabanpour,60*MES.Sbase_shabanpour,60*MES.Sbase_shabanpour,60*MES.Sbase_shabanpour,150,150,35*MW,35*MW]),xLF_init=np.array([20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,45*bar,47*bar,0,0,1*MES.Vbase_shabanpour,65,30,-60,20,20,200*MES.rho_w*MES.grav_const,100,120,120,50,50,50,10*MSCM*MES.rhon_g/hour,3*MSCM*MES.rhon_g/hour,3*MSCM*MES.rhon_g/hour,50*MES.Sbase_shabanpour,10*MES.Sbase_shabanpour,0*MES.Sbase_shabanpour,0*MES.Sbase_shabanpour,10,10,30*MW,25*MW]),a=np.array([0,0,0,0,0]),b=np.array([.01*MES.GHV,.3,.3,.04,.05]),c=np.array([1e-6*(MES.GHV)**2,3e-5,2e-5,4e-4,4.5e-4]),scale_var=None,scale_var_params=None,tol=1e-6,max_iter=50,formulation={'gas':'full','elec':'complex_power','heat':'half_link_flow','het':None},ineq_constr='control',derivatives=False,optimization_method='trust-constr',stay_within_bounds=False,fb=None,obj_fun='combined'):
    ...
    print('\nRunning OPF for MES Shabanpour (inequality constraints on: {}, analytical derivatives: {}, hard bounds: {}, method: {}, scaling: {})'.format(ineq_constr, derivatives,stay_within_bounds,optimization_method,scale_var))

    # create network
    with HiddenPrints():
        het_net, gas_net, elec_net, heat_net, _, _ = MES.create_network(hydr_eq='fa',heat_load='outflow',adjusted_coupling=False)

    # update the BC of the MES to match initial guess of OF
    pg2_init, V2_init, ph2_init, Ts2c_init = u_init
    het_net, gas_net, elec_net, heat_net = update_bc(het_net, gas_net, elec_net, heat_net, pg2_init, V2_init, ph2_init, Ts2c_init, scale_var=scale_var,scale_var_params=scale_var_params)
    ...
    if obj_fun == 'combined':
        ...
        y_ind = [len(u_init),xc_start_ind+3,xc_start_ind+4,xc_start_ind+9,xc_start_ind+10]
    elif obj_fun == 'gas':
        y_ind = [len(u_init)]
    # scaling
    if scale_var == 'matrix' or scale_var == 'per_unit':
        ...
        DF = nlsys.DF()
        if scale_var == 'matrix':
            ubase = np.array([scale_var_params.get('pgbase'),scale_var_params.get('Vbase'),scale_var_params.get('phbase'),scale_var_params.get('Tbase')])
        else:
            ubase = np.array([scale_var_params.get('pbase'),scale_var_params.get('Vbase'),scale_var_params.get('pbase'),scale_var_params.get('Tbase')])
    ...
    def eq_constr(y,nlsys=nlsys,scale_var=scale_var,scale_var_params=scale_var_params, Dy_inv=Dy_inv, Dh=Dh, formulation=formulation):
        ...
        pg2, V2, ph2, Ts2c = y[:len(u_init)] # scaled for p.u., unscaled for matrix
        network_mes, network_g, network_e, network_h = update_bc(network_mes, network_g, network_e, network_h, pg2, V2, ph2, Ts2c, scale_var=scale_var,scale_var_params=scale_var_params)
        H = h(y, nlsys=nlsys, Dh=Dh, scale_var=scale_var,scale_var_params=scale_var_params)
    ...
    def jac_eq_constr(y,nlsys=nlsys,scale_var=scale_var,scale_var_params=scale_var_params, Dy_inv=Dy_inv, Dh=Dh, formulation=formulation):
        ...
        dh_dy = h_der(y, nlsys=nlsys, Dy_inv=Dy_inv, Dh=Dh, scale_var=scale_var,scale_var_params=scale_var_params)
        return dh_dy
    ...
    # print solution
    pg2, V2, ph2, Ts2c = y_opf[:len(u_init)] # unscaled
    het_net, gas_net, elec_net, heat_net = update_bc(het_net, gas_net, elec_net, heat_net, pg2, V2, ph2, Ts2c)
    ...
    print('q = {} MSCM/hour'.format(q_vec/(MSCM*MES.rhon_g/hour)))
    print('q nodal inj = {} MSCM/hour'.format(q_inj/(MSCM*MES.rhon_g/hour)))
...
def get_u_from_net(het_net, gas_net, elec_net, heat_net,formulation={'gas':'full','elec':'complex_power','heat':'half_link_flow','het':None}, scale_var=None,scale_var_params=None):
    ...
    """
    pg2 = gas_net.nodes[2].get_p(scale_var=scale_var,scale_var_params=scale_var_params)
    V2 = elec_net.nodes[2].get_V(scale_var=scale_var,scale_var_params=scale_var_params)
    ph2 = heat_net.nodes[2].get_p(scale_var=scale_var,scale_var_params=scale_var_params)
    Ts2c = het_net.links[-1].get_Tsstart(scale_var=scale_var,scale_var_params=scale_var_params)
    return np.array([pg2, V2, ph2, Ts2c])
...
def solve_lf_in_of(het_net, gas_net, elec_net, heat_net, u, max_iters=10,tol=1e-6,formulation={'gas':'full','elec':'complex_power','heat':'half_link_flow','het':None}, scale_var=None,scale_var_params=None, xLF_init=np.array([20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,45*bar,47*bar,0,0,1*MES.Vbase_shabanpour,65,30,-60,20,20,200*MES.rho_w*MES.grav_const,100,120,120,50,50,50,10*MSCM*MES.rhon_g/hour,3*MSCM*MES.rhon_g/hour,3*MSCM*MES.rhon_g/hour,50*MES.Sbase_shabanpour,10*MES.Sbase_shabanpour,0*MES.Sbase_shabanpour,0*MES.Sbase_shabanpour,10,10,30*MW,25*MW])):
    ...
    # if len(err_LF_vec_global):
    #     print('||u-u_net||={:.3e}, err LF = {:.3e}'.format(np.linalg.norm(u-u_net),err_LF_vec_global[-1]))
    #     print('||u-u_net||>tol:{}, err LF > tol: {}, both: {}'.format(np.linalg.norm(u-u_net) > tol,err_LF_vec_global[-1] > tol,(np.linalg.norm(u-u_net) > tol) or (err_LF_vec_global[-1] > tol)))
    if len(err_LF_vec_global)==0 or (np.linalg.norm(u-u_net) > tol) or (err_LF_vec_global[-1] > tol):
            # print('Solving LF in OF')
            pg2, V2, ph2, Ts2c = u
            het_net, gas_net, elec_net, heat_net = update_bc(het_net, gas_net, elec_net, heat_net, pg2, V2, ph2, Ts2c, scale_var=scale_var,scale_var_params=scale_var_params)
            ...
            if err_vec[-1] >= tol:
                # print('LF in OF did not converge. Trying again with different initial guess.')
                het_net.reset_network(xLF_init,formulation=formulation,scale_var=scale_var,scale_var_params=scale_var_params)
                with HiddenPrints():
                    xmes,iters,err_vec,p_g_vec,q_vec,q_inj,delta_vec,V_mag_vec,S_inj,P_edge,Q_edge,m_vec,p_h_vec,Ts_vec,Tr_vec,m_hl_vec,phi_hl_vec,Ts_hl_vec,Tr_hl_vec,qc_vec,Pc_vec,Qc_vec,mc_vec,phic_vec,Tsc_vec,Trc_vec = het_net.solve_network(tol,max_iters,solver='NR',formulation=formulation,scale_var=scale_var,scale_var_params=scale_var_params)
            # print('Solved LF within OF: final error = {:.3e} after {} iterations (max iters = {})'.format(err_vec[-1],iters,max_iters))
            err_LF = err_vec[-1]
            q0 = q_inj[0]
            if scale_var == 'per_unit':
                q0 = q0/scale_var_params.get('qbase')
    else:
        # print('NOT solving LF in OF')
...
def run_optimal_load_flow_separate_LF(u_lb=np.array([1*bar,.8*MES.Vbase_shabanpour,1*MES.rho_w*MES.grav_const,100]),u_ub=np.array([MES.pg0,1.2*MES.Vbase_shabanpour,MES.ph0,135]),u_init=np.array([.9*MES.pg0,1.1*MES.Vbase_shabanpour,.9*MES.ph0,130]),slack_lb=np.array([-80*MSCM*MES.rhon_g/hour]),slack_ub=np.array([0]),slack_init=np.array([-50*MSCM*MES.rhon_g/hour]),xLF_lb=np.array([-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,1*bar,1*bar,-np.pi,-np.pi,0.8*MES.Vbase_shabanpour,-100,-100,-100,0,0,1*MES.rho_w*MES.grav_const,60,60,60,0,0,0,0,0,0,0,0,-50*MES.Sbase_shabanpour,-50*MES.Sbase_shabanpour,0,0,0,0]),xLF_ub=np.array([80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,60*bar,60*bar,np.pi,np.pi,1.2*MES.Vbase_shabanpour,100,100,100,200,200,6000*MES.rho_w*MES.grav_const,135,135,135,60,60,60,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,80*MSCM*MES.rhon_g/hour,60*MES.Sbase_shabanpour,60*MES.Sbase_shabanpour,60*MES.Sbase_shabanpour,60*MES.Sbase_shabanpour,150,150,35*MW,35*MW]),xLF_init=np.array([20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,20*MSCM*MES.rhon_g/hour,45*bar,47*bar,0,0,1*MES.Vbase_shabanpour,65,30,-60,20,20,200*MES.rho_w*MES.grav_const,100,120,120,50,50,50,10*MSCM*MES.rhon_g/hour,3*MSCM*MES.rhon_g/hour,3*MSCM*MES.rhon_g/hour,50*MES.Sbase_shabanpour,10*MES.Sbase_shabanpour,0*MES.Sbase_shabanpour,0*MES.Sbase_shabanpour,10,10,30*MW,25*MW]),a=np.array([0,0,0,0,0]),b=np.array([.01*MES.GHV,.3,.3,.04,.05]),c=np.array([1e-6*(MES.GHV)**2,3e-5,2e-5,4e-4,4.5e-4]),scale_var=None,scale_var_params=None,tol=1e-6,max_iter=50,max_iters_lf=10,formulation={'gas':'full','elec':'complex_power','heat':'half_link_flow','het':None},ineq_constr='control',approach='direct',optimization_method='trust-constr',stay_within_bounds=False,fb=None,obj_fun='combined'):
    ...
    if formulation.get('heat') != 'half_link_flow':
        raise ValueError('OF not implemented for heat-formulation other than half_link_flow')
    print('\nRunning OF for MES (inequality constraints on: {}, approach: {}, hard bounds: {}, method: {}, scaling: {})'.format(ineq_constr, approach,stay_within_bounds,optimization_method,scale_var))
    ...
    # update the BC of the MES to match initial guess of OF
    pg2_init, V2_init, ph2_init, Ts2c_init = u_init
    het_net, gas_net, elec_net, heat_net = update_bc(het_net, gas_net, elec_net, heat_net, pg2_init, V2_init, ph2_init, Ts2c_init)  #unscaled
    # set intial guess and initialize network
    het_net.initialize()
    het_net.update(xLF_init,formulation=formulation)
    # het_net.set_x_init(formulation=formulation,scale_var=scale_var,scale_var_params=scale_var_params)
    ...
    if obj_fun == 'combined':
        ...
        y_ind = [len(u_init)]
    # print('y_ind = {} (it should be [4] or [4,29,30,35,36])'.format(y_ind))
    # scaling
    if scale_var == 'matrix' or scale_var == 'per_unit':
        Dx = nlsys.Dx()
        Dx_inv = np.diag(1/Dx.data[0])
    ...
    def obj(u,y_ind=y_ind,a=a,b=b,c=c,nlsys=nlsys,scale_var=scale_var,scale_var_params=scale_var_params, fb=fb, formulation=formulation, Du_inv=Du_inv, max_iters_lf=max_iters_lf,tol=tol,xLF_init=xLF_init):
        ...
        u_f_vec = u.copy()
        global f_vec_global
        # print('In the {}th call to obj()'.format(len(f_vec_global)))
        # print('Variables u before unscaling: {}'.format(u))
        if scale_var == 'matrix':
            # Optimizer uses scaled y, but objective function wants unscaled y (when using matrix scaling)
            u = Du_inv.dot(u)
        # print('Variables u used to solve lf: {}'.format(u))
        ...
        # print('Variables u given to obj: {}'.format(u))
        # print('Variables given to obj: {}'.format(y[np.array(y_ind)]))
        f = objective_function(y,y_ind,a=a,b=b,c=c,scale_var=scale_var,scale_var_params=scale_var_params,fb=fb)
        f_vec_global.append(f)
        return f
    ...
    def obj_grad(u,y_ind=y_ind,a=a,b=b,c=c,nlsys=nlsys,scale_var=scale_var,scale_var_params=scale_var_params, fb=fb, formulation=formulation, max_iters_lf=max_iters_lf,tol=tol,method=approach,xLF_init=xLF_init):
        ...
        V2 = u[1]
        nlsys.nlsystemse[0].V_vec_mag[2] = V2
        deltah_deltay = h_der(y, nlsys=nlsys, Dy_inv=Dy_inv, Dh=Dh, scale_var=scale_var,scale_var_params=scale_var_params) # also resets network as a first step
...
def par_obj(obj_fun):
    ...
    if obj_fun == 'gas':
        ...
        y_ind = [y_ind_gas,29,30,35,36]
    return a,b,c,y_ind

def bounds_OF():
    """Returns the lower and upper bounds for the control variables u, the slack varialbes, and the state variables x"""
    u_lb=np.array([1*bar,.8*MES.Vbase_shabanpour,1*MES.rho_w*MES.grav_const,100])
    u_ub=np.array([60*bar,1.2*MES.Vbase_shabanpour,6000*MES.rho_w*MES.grav_const,135])
    slack_lb=np.array([-80*MSCM*MES.rhon_g/hour])
    slack_ub=np.array([-(10.8649+20)*MSCM*MES.rhon_g/hour])
    xLF_lb=np.array([-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,-80*MSCM*MES.rhon_g/hour,1*bar,1*bar,-np.pi,-np.pi,0.8*MES.Vbase_shabanpour,-100,-100,-100,0,0,1*MES.rho_w*MES.grav_const,60,60,60,0,0,0,0,0,0,0,0,-50*MES.Sbase_shabanpour,-50*MES.Sbase_shabanpour,0,0,0,0])
...
def initial_guess():
    ...
    u_init=np.array([.9*MES.pg0,1.1*MES.Vbase_shabanpour,.9*MES.ph0,130])
...
def compare_methods(dir_path=None,save_tables=False,save_figs=False):
    """Compare OF of MES network for different optimization methods, objective function, and bounds. Without scaling."""
    ...
    with HiddenPrints():
        het_net_LF, gas_net_LF, elec_net_LF, heat_net_LF, water, gas = MES.create_network(hydr_eq='fa',heat_load='outflow',adjusted_coupling=False)
        x0_LF = MES.initialize_network(het_net_LF,gas_net_LF,elec_net_LF,heat_net_LF,water,gas,formulation,heat_load='outflow',scale_var=None,scale_var_params=None)
        xmes_LF, iters_LF, err_vec_LF,p_g_vec,_,q_inj,_,V_mag_vec,_,_,_,_,p_h_vec,_,_,_,_,_,_,_,_,_,_,_,Tsc_vec,_ = het_net_LF.solve_network(tol,10,solver='NR',formulation=formulation)
    pg2_sol = p_g_vec[2]
    V2_sol = V_mag_vec[2]
    ph2_sol = p_h_vec[2]
    Ts2c_sol = Tsc_vec[1]
    q0_sol = q_inj[0]
    y_LF = np.concatenate((np.array([pg2_sol,V2_sol,ph2_sol,Ts2c_sol,q0_sol]),xmes_LF))
    ...
    for obj_fun in obj_funs:
        ...
        for ineq_constr in ineq_constrs:
            # plots
            fig_f = plt.figure('obj_OF_MES_ineq_constr_{}_obj_{}'.format(ineq_constr,obj_fun))
            ax_f = fig_f.gca()
    ...
    if save_tables:
        ...
        with open(os.path.join(path_to_tables,'optimizer_info_MES_methods.txt'), "w") as table:
            for obj_fun in obj_funs:
                for ineq_constr in ineq_constrs:
                    for bound in bounds:
                        for der in ders:
...
def compare_methods_scaled(dir_path=None,save_tables=False,save_figs=False):
    """Compare OF of MES for different optimization methods, objective function, and bounds. Using matrix scaling."""
    if (save_tables or save_figs) and dir_path == None:
        raise ValueError('dir_path needs to be specified to save tables or figures')
    ...
    deltabase = 1.
    scale_var_params={'qbase':qbase,'pgbase':pgbase,'pbase':pgbase,'Sbase':Sbase,'Vbase':Vbase,'deltabase':deltabase,'mbase':mbase,'phbase':phbase,'Tbase':Tbase,'phibase':phibase,'Ebase':Sbase}
    ...
    slack_base = np.array([scale_var_params.get('qbase')])
    Dy = np.diag(np.concatenate((1/ubase,1/slack_base,Dx.data[0])))
    y_LF = np.concatenate((np.array([pg2_sol,V2_sol,ph2_sol,Ts2c_sol,q0_sol]),xmes_LF)) #unscaled
    # value of objective functions for LF solution
    a_gas,b_gas,c_gas,y_ind_gas = par_obj('gas')
    ...
    for obj_fun in obj_funs:
        ...
        for ineq_constr in ineq_constrs:
            # plots
            fig_f = plt.figure('obj_OF_MES_ineq_constr_{}_obj_{}_scaled'.format(ineq_constr,obj_fun))
            ax_f = fig_f.gca()
            ax_f.set_xlabel('Iteration')
            ax_f.set_ylabel('f')
    ...
    if save_tables:
        path_to_tables = os.path.join(dir_path,'Tables','N_Shabanpour')
        with open(os.path.join(path_to_tables,'optimizer_info_MES_methods_scaled.txt'), "w") as table:
            for obj_fun in obj_funs:
                for ineq_constr in ineq_constrs:
                    for bound in bounds:
                        for der in ders:
...
def compare_methods_sep_LF(dir_path=None,save_tables=False,save_figs=False):
    """Compare OF of MES network for different optimization methods, objective function, and bounds. LF is included implicitly. Without scaling."""
    if (save_tables or save_figs) and dir_path == None:
        raise ValueError('dir_path needs to be specified to save tables or figures')
    ...
    for obj_fun in obj_funs:
        ...
        for ineq_constr in ineq_constrs:
            # plots
            fig_f = plt.figure('obj_OF_MES_ineq_constr_{}_obj_{}_sep_LF'.format(ineq_constr,obj_fun))
            ax_f = fig_f.gca()
            ax_f.set_xlabel('Iteration')
            ax_f.set_ylabel('f')

            fig_LF_error = plt.figure('error_LF_in_OF_MES_ineq_constr_{}_obj_{}_sep_LF'.format(ineq_constr,obj_fun))
    ...
    if save_tables:
        path_to_tables = os.path.join(dir_path,'Tables','N_Shabanpour')
        with open(os.path.join(path_to_tables,'optimizer_info_MES_methods_sep_LF.txt'), "w") as table:
            for obj_fun in obj_funs:
                for ineq_constr in ineq_constrs:
                    for bound in bounds:
                        for approach in approaches:
        ...
        for obj_fun in obj_funs:
            if obj_fun == 'gas':
                ...
                y_ind = y_ind_comb
            var_ind = np.concatenate((np.array(range(len(u_init))),y_ind))
            for ineq_constr in ineq_constrs:
                for bound in bounds:
                    y_trust = y_res.get('trust-constr_'+bound+'_'+ineq_constr+'_'+obj_fun+'_eq_constr')
                    y_slsqp = y_res.get('SLSQP_'+bound+'_'+ineq_constr+'_'+obj_fun+'_eq_constr')
                    y_ipopt = y_res.get('ipopt_'+bound+'_'+ineq_constr+'_'+obj_fun+'_eq_constr')
                    y_trust_dir = y_res.get('trust-constr_'+bound+'_'+ineq_constr+'_'+obj_fun+'_direct')
                    y_slsqp_dir = y_res.get('SLSQP_'+bound+'_'+ineq_constr+'_'+obj_fun+'_direct')
                    y_ipopt_dir = y_res.get('ipopt_'+bound+'_'+ineq_constr+'_'+obj_fun+'_direct')
                    y_trust_adj = y_res.get('trust-constr_'+bound+'_'+ineq_constr+'_'+obj_fun+'_adjoint')
                    y_slsqp_adj = y_res.get('SLSQP_'+bound+'_'+ineq_constr+'_'+obj_fun+'_adjoint')
                    y_ipopt_adj = y_res.get('ipopt_'+bound+'_'+ineq_constr+'_'+obj_fun+'_adjoint')
                    with open(os.path.join(path_to_tables,'network_solution_MES_methods_sep_LF_{}_{}_{}.txt'.format(obj_fun,ineq_constr,bound)), "w") as table:
                        for ind_var in var_ind:
                            table.write(r'{} & {:.3f} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} \\ '.format(variable_names[ind_var],y_LF[ind_var],error(y_trust[ind_var],y_LF[ind_var]),error(y_slsqp[ind_var],y_LF[ind_var]),error(y_ipopt[ind_var],y_LF[ind_var]),error(y_trust_dir[ind_var],y_LF[ind_var]),error(y_slsqp_dir[ind_var],y_LF[ind_var]),error(y_ipopt_dir[ind_var],y_LF[ind_var]),error(y_trust_adj[ind_var],y_LF[ind_var]),error(y_slsqp_adj[ind_var],y_LF[ind_var]),error(y_ipopt_adj[ind_var],y_LF[ind_var])))
                        table.write(r'\hline ')
                        table.write(r'\multicolumn{2}{|c|}{max error $\vec{y}$} '+r' & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} \\ '.format(error(y_trust,y_LF),error(y_slsqp,y_LF),error(y_ipopt,y_LF),error(y_trust_dir,y_LF),error(y_slsqp_dir,y_LF),error(y_ipopt_dir,y_LF),error(y_trust_adj,y_LF),error(y_slsqp_adj,y_LF),error(y_ipopt_adj,y_LF)))
                        table.write(r'\multicolumn{2}{|c|}{in variable} '+r' & {} & {} & {} & {} & {} & {} & {} & {} & {} \\ '.format(variable_names[max_error_ind(y_trust,y_LF)],variable_names[max_error_ind(y_slsqp,y_LF)],variable_names[max_error_ind(y_ipopt,y_LF)],variable_names[max_error_ind(y_trust_dir,y_LF)],variable_names[max_error_ind(y_slsqp_dir,y_LF)],variable_names[max_error_ind(y_ipopt_dir,y_LF)],variable_names[max_error_ind(y_trust_adj,y_LF)],variable_names[max_error_ind(y_slsqp_adj,y_LF)],variable_names[max_error_ind(y_ipopt_adj,y_LF)]))
...
def compare_methods_sep_LF_scaled(dir_path=None,save_tables=False,save_figs=False):
    """Compare OF of MES network for different optimization methods, objective function, and bounds. LF is included implicitly. Using matrix scaling."""
    if (save_tables or save_figs) and dir_path == None:
        raise ValueError('dir_path needs to be specified to save tables or figures')
    ...
    for obj_fun in obj_funs:
        ...
        for ineq_constr in ineq_constrs:
            # plots
            fig_f = plt.figure('obj_OF_MES_ineq_constr_{}_obj_{}_sep_LF_scaled'.format(ineq_constr,obj_fun))
            ax_f = fig_f.gca()
            ax_f.set_xlabel('Iteration')
            ax_f.set_ylabel('f')

            fig_LF_error = plt.figure('error_LF_in_OF_MES_ineq_constr_{}_obj_{}_sep_LF_scaled'.format(ineq_constr,obj_fun))
            ax_LF_error = fig_LF_error.gca()
            ax_LF_error.set_xlabel('Iteration?')
            ax_LF_error.set_ylabel(r'$||F||_2$')
    ...
    if save_tables:
        path_to_tables = os.path.join(dir_path,'Tables','N_Shabanpour')
        with open(os.path.join(path_to_tables,'optimizer_info_MES_methods_sep_LF_scaled.txt'), "w") as table:
            for obj_fun in obj_funs:
                for ineq_constr in ineq_constrs:
                    for bound in bounds:
                        for approach in approaches:
        ...
        for obj_fun in obj_funs:
            ...
            for ineq_constr in ineq_constrs:
                for bound in bounds:
                    ...
                    y_slsqp_adj = y_res.get('SLSQP_'+bound+'_'+ineq_constr+'_'+obj_fun+'_adjoint')
                    y_ipopt_adj = y_res.get('ipopt_'+bound+'_'+ineq_constr+'_'+obj_fun+'_adjoint')
                    with open(os.path.join(path_to_tables,'network_solution_MES_methods_sep_LF_scaled_{}_{}_{}.txt'.format(obj_fun,ineq_constr,bound)), "w") as table:
                        for ind_var in var_ind:
                            table.write(r'{} & {:.3f} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} \\ '.format(variable_names[ind_var],y_LF[ind_var],error(y_trust[ind_var],y_LF[ind_var]),error(y_slsqp[ind_var],y_LF[ind_var]),error(y_ipopt[ind_var],y_LF[ind_var]),error(y_trust_dir[ind_var],y_LF[ind_var]),error(y_slsqp_dir[ind_var],y_LF[ind_var]),error(y_ipopt_dir[ind_var],y_LF[ind_var]),error(y_trust_adj[ind_var],y_LF[ind_var]),error(y_slsqp_adj[ind_var],y_LF[ind_var]),error(y_ipopt_adj[ind_var],y_LF[ind_var])))
                        table.write(r'\hline ')
                        table.write(r'\multicolumn{2}{|c|}{max error $\vec{y}$} '+r' & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} \\ '.format(error(y_trust,y_LF),error(y_slsqp,y_LF),error(y_ipopt,y_LF),error(y_trust_dir,y_LF),error(y_slsqp_dir,y_LF),error(y_ipopt_dir,y_LF),error(y_trust_adj,y_LF),error(y_slsqp_adj,y_LF),error(y_ipopt_adj,y_LF)))
...
def compare_methods_sep_LF_scaled_pu(dir_path=None,save_tables=False,save_figs=False):
    """Compare OF of MES network for different optimization methods, objective function, and bounds. LF is included implicitly. Using per unit scaling."""
    if (save_tables or save_figs) and dir_path == None:
        raise ValueError('dir_path needs to be specified to save tables or figures')
    ...
    Vbase = 1.*MES.Vbase_shabanpour
    deltabase = 1.
    scale_var_params={'qbase':mbase,'pgbase':pbase,'pbase':pbase,'Sbase':Sbase,'Vbase':Vbase,'deltabase':deltabase,'mbase':mbase,'phbase':pbase,'Tbase':Tbase,'phibase':phibase,'Ebase':Sbase}
    ...
    # value of objective functions for LF solution
    a_gas,b_gas,c_gas,y_ind_gas = par_obj('gas')
    f_LF_gas = objective_function(y_LF,y_ind_gas,a=a_gas/fb,b=b_gas/(fb/np.diag(Dy_inv)[np.array(y_ind_gas)]),c=c_gas/(fb/np.diag(Dy_inv)[np.array(y_ind_gas)]**2)) # scaled
    a_comb,b_comb,c_comb,y_ind_comb = par_obj('combined')
    f_LF_comb = objective_function(y_LF,y_ind_comb,a=a_comb/fb,b=b_comb/(fb/np.diag(Dy_inv)[np.array(y_ind_comb)]),c=c_comb/(fb/np.diag(Dy_inv)[np.array(y_ind_comb)]**2)) # scaled
    ...
    for obj_fun in obj_funs:
        ...
        for ineq_constr in ineq_constrs:
            # plots
            fig_f = plt.figure('obj_OF_MES_ineq_constr_{}_obj_{}_sep_LF_scaled_pu'.format(ineq_constr,obj_fun))
            ax_f = fig_f.gca()
            ax_f.set_xlabel('Iteration')
            ax_f.set_ylabel('f')

            fig_LF_error = plt.figure('error_LF_in_OF_MES_ineq_constr_{}_obj_{}_sep_LF_scaled_pu'.format(ineq_constr,obj_fun))
            ax_LF_error = fig_LF_error.gca()
            ax_LF_error.set_xlabel('Iteration?')
            ax_LF_error.set_ylabel(r'$||F||_2$')
    ...
    if save_tables:
        path_to_tables = os.path.join(dir_path,'Tables','N_Shabanpour')
        with open(os.path.join(path_to_tables,'optimizer_info_MES_methods_sep_LF_scaled_pu.txt'), "w") as table:
            for obj_fun in obj_funs:
                for ineq_constr in ineq_constrs:
                    for bound in bounds:
                        for approach in approaches:
        ...
        for obj_fun in obj_funs:
            ...
            for ineq_constr in ineq_constrs:
                for bound in bounds:
                    ...
                    y_slsqp_adj = y_res.get('SLSQP_'+bound+'_'+ineq_constr+'_'+obj_fun+'_adjoint')
                    y_ipopt_adj = y_res.get('ipopt_'+bound+'_'+ineq_constr+'_'+obj_fun+'_adjoint')
                    with open(os.path.join(path_to_tables,'network_solution_MES_methods_sep_LF_scaled_pu_{}_{}_{}.txt'.format(obj_fun,ineq_constr,bound)), "w") as table:
                        for ind_var in var_ind:
                            table.write(r'{} & {:.3f} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} \\ '.format(variable_names[ind_var],y_LF[ind_var],error(y_trust[ind_var],y_LF[ind_var]),error(y_slsqp[ind_var],y_LF[ind_var]),error(y_ipopt[ind_var],y_LF[ind_var]),error(y_trust_dir[ind_var],y_LF[ind_var]),error(y_slsqp_dir[ind_var],y_LF[ind_var]),error(y_ipopt_dir[ind_var],y_LF[ind_var]),error(y_trust_adj[ind_var],y_LF[ind_var]),error(y_slsqp_adj[ind_var],y_LF[ind_var]),error(y_ipopt_adj[ind_var],y_LF[ind_var])))
                        table.write(r'\hline ')
                        table.write(r'\multicolumn{2}{|c|}{max error $\vec{y}$} '+r' & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} & {:.3e} \\ '.format(error(y_trust,y_LF),error(y_slsqp,y_LF),error(y_ipopt,y_LF),error(y_trust_dir,y_LF),error(y_slsqp_dir,y_LF),error(y_ipopt_dir,y_LF),error(y_trust_adj,y_LF),error(y_slsqp_adj,y_LF),error(y_ipopt_adj,y_LF)))
...
if __name__ == '__main__':
    ...
    with warnings.catch_warnings():
        ...
        # compare_methods_sep_LF_scaled_pu(dir_path=dir_path,save_tables=False,save_figs=False)
